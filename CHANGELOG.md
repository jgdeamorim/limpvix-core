...
### Added
- **CORS Support**: Cross-Origin Resource Sharing for web and mobile apps
  - `CorsService`: Manages CORS headers and preflight requests
  - Configurable allowed origins via WordPress options
  - Pattern matching support (e.g., *.example.com)
  - Automatic preflight OPTIONS handling
  - 24-hour cache for preflight responses
- **API Documentation**: Complete REST API documentation
  - Authentication guide (JWT, API Keys, Application Passwords)
...
### Changed
- Updated `AuthBootstrap` to register CORS filters (priority 0 - runs first)
- CORS headers added to all /limpvix/v1/* responses
- Preflight requests return 200 OK with appropriate headers

### Security
- CORS origin validation (exact match or pattern)
- Wildcard support disabled by default in production
...
### Added
- **API Key Authentication System**: Alternative authentication method for external applications
  - `ApiKey`: Value object representing API keys with scopes and expiration
  - `ApiKeyService`: Manages API key lifecycle (create, validate, revoke, delete)
  - `ApiKeyAuthMiddleware`: Authenticates requests via X-API-Key header
  - `ApiKeyController`: REST endpoints for managing API keys
  - Storage: WordPress options with key hashing (SHA256)
  - Scopes: read, write, admin, or * (wildcard for all)
- **REST API Endpoints**:
  - `GET /limpvix/v1/api-keys` - List user API keys
  - `POST /limpvix/v1/api-keys` - Create new API key
  - `DELETE /limpvix/v1/api-keys/{key}` - Revoke API key
- **Multi-Method Authentication**: Requests can use JWT, API Key, or WordPress session
  - Priority: API Key > JWT > Session
  - Automatic detection via headers

### Changed
- Updated `AuthBootstrap` to register API key authentication and endpoints
- API key auth runs before JWT auth (priority 5)

### Security
- API keys stored as SHA256 hash (not plaintext)
- Keys shown in full only once during creation
- Masked keys in list responses (first 20 chars + ...)
...
### Added
- **Rate Limiting System**: Token bucket algorithm with sliding window
  - `RateLimitService`: Core service for managing rate limits per client
  - `RateLimitMiddleware`: REST API middleware for enforcing limits
  - Three-tier system: Public (60 req/min), Authenticated (300 req/min), Admin (1000 req/min)
  - Automatic token refill over time
  - Storage using WordPress transients
- **Rate Limit Headers**: X-RateLimit-* headers on all API responses
  - `X-RateLimit-Limit`: Maximum requests allowed
  - `X-RateLimit-Remaining`: Requests remaining in window
  - `X-RateLimit-Reset`: Unix timestamp when limit resets
- **Client Identification**: IP-based and user-based rate limiting with fallback
- **Integration Tests**: 12 tests for rate limiting with 100% pass rate

### Changed
- Updated `AuthBootstrap` to register rate limiting filters on REST API
- Rate limiting automatically applied to all `/limpvix/v1/*` endpoints

### Security
- Protection against API abuse and DoS attacks
- Different limits for public vs authenticated users
- Admin users get higher limits for management tasks

## [0.7.0] - 2026-02-12

### Added
- **JWT Authentication System**: Implemented stateless token-based authentication for REST API
  - `JwtService`: Core service for generating and validating JWT tokens
  - `JwtAuthMiddleware`: Middleware for REST API permission callbacks
  - `AuthController`: REST endpoints for login, token refresh, and user info
  - Access tokens (1 hour TTL) and refresh tokens (30 days TTL)
  - Hybrid authentication support (JWT + WordPress session fallback)
- **Integration Tests**: 15 tests for JWT authentication with 100% pass rate
- **API Endpoints**:
  - `POST /limpvix/v1/auth/login` - User authentication with JWT token generation
  - `POST /limpvix/v1/auth/refresh` - Refresh expired access tokens
  - `GET /limpvix/v1/auth/me` - Get authenticated user information

### Changed
- Updated existing REST API controllers to support JWT authentication:
  - `BriefingController`: Added JWT middleware support
  - `ContractController`: Prepared for JWT authentication
  - `ExecutionController`: Fixed duplicate method declarations
- Modified `Kernel.php` to initialize authentication system

### Fixed
- Removed duplicate methods in `ExecutionController` (removeEvidence, listEvidences)
- Fixed permission callbacks to work with both JWT and session authentication

### Security
- Implemented HS256 algorithm for token signing
- Added CSRF protection via state parameter validation
- Secret key stored in WordPress options (should be moved to wp-config.php in production)
