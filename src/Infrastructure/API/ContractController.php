...
class ContractController
{
     ...
     */
    public function __construct(array $useCases = [], ?AuthorizationService $authService = null, ?JwtAuthMiddleware $jwtMiddleware = null)
    {
    ...
    public function register_routes(): void
    {
        ...
        // GET /limpvix/v1/contracts/{id}/reallocation-options
        register_rest_route($this->namespace, '/contracts/(?P<id>\d+)/reallocation-options', [
            'methods' => 'GET',
            'callback' => [$this, 'getReallocationOptions'],
            'permission_callback' => [$this, 'checkAdminPermissions']
        ]);

        // POST /limpvix/v1/contracts/{id}/reallocate
        register_rest_route($this->namespace, '/contracts/(?P<id>\d+)/reallocate', [
            'methods' => 'POST',
            'callback' => [$this, 'reallocateProfessional'],
            'permission_callback' => [$this, 'checkAdminPermissions'],
            'args' => [
                'new_professional_id' => [
                    'required' => true,
    ...
    /**
     * GET /limpvix/v1/contracts/{id}/reallocation-options
     * Get list of eligible professionals for reallocation with match scores
     */
    public function getReallocationOptions(WP_REST_Request $request): WP_REST_Response
    {
        if (!isset($this->useCases['get_reallocation_options'])) {
            return ApiResponse::error('GetReallocationOptions Use Case not available', null, 500);
        }
        ...
        try {
            $options = $this->useCases['get_reallocation_options']->execute($contractId, $limit);
            ...
            return ApiResponse::error(
                'Failed to get reallocation options: ' . $e->getMessage(),
                null,
                500
            );
        }
    ...
    /**
     * POST /limpvix/v1/contracts/{id}/reallocate
     * Reallocate professional to a different one
     */
    public function reallocateProfessional(WP_REST_Request $request): WP_REST_Response
    {
        if (!isset($this->useCases['reallocate_professional'])) {
            return ApiResponse::error('ReallocateProfessional Use Case not available', null, 500);
        }

        $contractId = (int) $request->get_param('id');
        $newProfessionalId = (int) $request->get_param('new_professional_id');
        $reason = trim($request->get_param('reason'));
        $adminUserId = get_current_user_id();

        try {
            $result = $this->useCases['reallocate_professional']->execute(
                $contractId,
                $newProfessionalId,
                $reason,
                $adminUserId
            ...
            return ApiResponse::error(
                'Failed to reallocate professional: ' . $e->getMessage(),
                null,
                500
            );
        }
