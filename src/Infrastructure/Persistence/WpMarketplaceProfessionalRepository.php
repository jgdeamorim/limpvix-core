...
class WpMarketplaceProfessionalRepository implements ProfessionalRepositoryInterface
{
     ...
     *
     * Queries feedback history to detect patterns of poor performance.
     * Used by OnConsecutivePoorFeedback listener to trigger reallocation.
     *
     * @param int $professionalId Professional ID
     * @param float $threshold Rating threshold (default 3.0 - below is "poor")
     * @param int $count Minimum consecutive poor feedbacks to return
    ...
    public function findConsecutivePoorFeedbacks(
        ...
        $feedbackTable = $this->wpdb->prefix . 'limpvix_structured_feedbacks';
        $executionTable = $this->wpdb->prefix . 'limpvix_executions';

        // Query to find poor feedbacks for this professional
        $sql = $this->wpdb->prepare(
            "SELECT sf.*, e.professional_id
             FROM {$feedbackTable} sf
             INNER JOIN {$executionTable} e ON sf.order_uuid = e.order_uuid
             WHERE e.professional_id = %d
               AND sf.status = 'validated'
               AND sf.final_score < %f
               AND sf.submitted_at > DATE_SUB(NOW(), INTERVAL %d DAY)
             ORDER BY sf.submitted_at DESC
             LIMIT %d",
            $professionalId,
            $threshold,
            $days,
            $count * 2  // Fetch more to ensure we have enough
        );

        $results = $this->wpdb->get_results($sql, ARRAY_A);

        // Check if they are consecutive (no good feedback in between)
        if (empty($results)) {
            return [];
        }

        // Return all poor feedbacks found (listener will check count)
        return $results;
    }
}
