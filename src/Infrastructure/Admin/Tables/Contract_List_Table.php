...
class Contract_List_Table extends \WP_List_Table
{
    ...
    private array $usersCache = []; // Cache de usuários para evitar N+1 queries
    ...
    public function prepare_items(): void
    {
        ...
        // OTIMIZAÇÃO: Pré-carregar usuários para evitar N+1 queries
        $this->preloadUsers($this->items);
    ...
    public function column_default($item, $column_name): string
    {
        switch ($column_name) {
            ...
            case 'client':
                ...
                // OTIMIZAÇÃO: Usar cache de usuários ao invés de get_user_by()
                $user = $this->usersCache[$userId] ?? null;
    ...
    /**
     * Pré-carregar usuários para evitar N+1 queries
     * OTIMIZAÇÃO: Carrega todos os usuários necessários em UMA query
     *
     * @param array $items Lista de contratos
     * @return void
     */
    private function preloadUsers(array $items): void
    {
        ...
        $userIds = array_unique(array_filter(array_map(function($item) {
            return (int) ($item['client_user_id'] ?? 0);
        }, $items)));
        ...
        // Armazenar em cache indexado por ID
